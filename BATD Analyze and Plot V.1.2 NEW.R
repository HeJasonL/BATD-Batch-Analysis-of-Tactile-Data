...
#Plot with text
plots_SFD <- ggarrange(plot_SFD1and2, text_SFD, ncol = 2, nrow = 1)
plots_SFD <- annotate_figure(plots_SFD, top = text_grob("Simultaneous Frequency Discrimination Threshold", color = "black", face = "bold"))
}} else {
accuracy_SFD <- NA
medianRT_SFD <- NA
threshold_SFD <- NA
...
#Plot with text
plots_SQFD <- ggarrange(plot_SQFD1and2, text_SQFD, ncol = 2, nrow = 1)
plots_SQFD <- annotate_figure(plots_SQFD, top = text_grob("Sequential Frequency Discrimination Threshold", color = "black", face = "bold"))
}} else{
accuracy_SQFD <- NA
medianRT_SQFD <- NA
threshold_SQFD <- NA
...
plots_TOJ <- annotate_figure(plots_TOJ, top = text_grob("Simultaneous Frequency Discrimination Threshold", color = "black", face = "bold"))
plot_TOJ1 <- ggplot(Data, aes(x=trialNumber, y=value, color=as.factor(correctResponse)))+
geom_point(size =3) +theme(legend.position="none") +
labs(y = "Interstimulus interval (ms)", x = "Trial Number")
...
participantPerformance <- as.data.frame(rbindlist(participant_performance)) #Combines output from the for loop above
